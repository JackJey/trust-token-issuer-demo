...
app.post(`/.well-known/trust-token/send-srr`, async (req, res) => {
  ...
  const srr_verify     = await ed25519.verify(signature.value, body.value, srr_public_key)
  ...
  const public_key_verify      = ({client_public_key: client_public_key_hash.toString() === key_hash.toString()})
